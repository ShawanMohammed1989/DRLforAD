...
class CustomModel(TFModelV2):
    ...
    def __init__(self, obs_space, action_space, num_outputs, model_config, name):
        ...
        self.save_dummy = tf.keras.Model([self.vae_encoder, self.vae_decoder, self.actor_critic_shared_model, self.actor_model, self.critic_model])
        self.vae_dummy = tf.keras.Model([self.vae_encoder, self.vae_decoder])
    ...
    def forward(self, input_dict, state, seq_lens):
        ...
        inputs = [tf.cast(image, dtype='float32')]

        if self.configuration['CONCAT_SPEED']:
            inputs.append(tf.cast(input_dict['obs']['speed']))
    
        if self.configuration['CONCAT_STEERING']:
            inputs.append(tf.cast(input_dict['obs']['steering'], dtype='float32'))
    
        if self.configuration['CONCAT_GYRO']:
            inputs.append(tf.cast(input_dict['obs']['gyro'], dtype='float32'))
        
        mean, _, vae_feature_map_extraction = self.vae_encoder(inputs)
    ...
    def import_from_h5(self, h5_file: str) -> None:
        if self.configuration.get("SAVED_MODEL_WEIGHTS_FILE"):
            self.save_dummy.load_weights(h5_file)
        if self.configuration.get("SAVED_VAE_WEIGHTS_FILE"):
            self.vae_dummy.load_weights(h5_file)
